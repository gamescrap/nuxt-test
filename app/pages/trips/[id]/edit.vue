...
watch(trip, (val) => {
  if (!val) return
  const dt = new Date(val.tripDatetime)
  form.tripDate     = val.tripDatetime.slice(0, 10)
  form.tripHour     = String(dt.getHours()).padStart(2, '0')
  form.tripMinute   = String(dt.getMinutes()).padStart(2, '0')
  form.availableSeats = val.availableSeats
  form.smokingAllowed = val.smokingAllowed

  // Pré-remplit les adresses
  departure.selectCity(val.departureAddress.city)
  departure.selectAddress(val.departureAddress)
  arriving.selectCity(val.arrivingAddress.city)
  arriving.selectAddress(val.arrivingAddress)
}, { immediate: true })
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (departure.selectedAddress.value?.id) {
      body.departureAddressId = departure.selectedAddress.value.id
    }
    if (arriving.selectedAddress.value?.id) {
      body.arrivingAddressId = arriving.selectedAddress.value.id
    }
    await updateTrip(Number(route.params.id), body)
    await navigateTo(`/trips/${route.params.id}`)
  } catch (e: any) {
  ...
  <main class="min-h-[calc(100vh-113px)] md:min-h-[calc(100vh-130px)] bg-gray-50">
    <div class="max-w-2xl mx-auto px-4 py-6 space-y-4">
      ...
      <nav class="flex items-center gap-2 text-sm mb-2">
        <NuxtLink :to="`/trips/${route.params.id}`" class="text-gray-400 hover:text-gray-700 transition-colors">
          Détail du trajet
        </NuxtLink>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
          ...
          <AddressSearchInput
              ...
              @update:address-query="q => { departure.addressQuery.value = q; departure.searchAddresses(q) }"
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
          ...
          <AddressSearchInput
              ...
              @update:address-query="q => { arriving.addressQuery.value = q; arriving.searchAddresses(q) }"
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4 space-y-4">
          ...
          <div v-if="form.tripDate" class="space-y-1">
            ...
            <div class="flex items-center gap-2">
              <select v-model="form.tripHour" class="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">
                <option v-for="h in hours" :key="h" :value="h">{{ h }}h</option>
              </select>
              <span class="text-gray-400 font-medium">:</span>
              <select v-model="form.tripMinute" class="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">
